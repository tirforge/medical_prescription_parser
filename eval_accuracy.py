...
def main():
    ...
    for img in files:
        ...
        rows.append({"file": base, "meds": meds, "med_hit": round(med_hit,2), "patient": patient, "patient_hit": patient_hit})
        print(f"{base}: meds={meds} hit={med_hit:.2f} patient='{patient}' hit={patient_hit}")
    avg_med = sum(r.get("med_hit",0) for r in rows)/max(1,len(rows))
    avg_pat = sum(r.get("patient_hit",0) for r in rows)/max(1,len(rows))
    print(f"\nOverall med hit: {avg_med:.2f}  patient hit: {avg_pat:.2f}  n={len(rows)}")
    json.dump({"avg_med_hit": avg_med, "avg_patient_hit": avg_pat, "rows": rows}, open("eval_results.json","w"), indent=2)
